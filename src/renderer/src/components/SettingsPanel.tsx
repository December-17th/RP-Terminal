          ...
          <>
            ...
            {settings.memory?.enabled && (
              <>
                ...
                <select
                  value={settings.memory?.utility_api_preset_id ?? ''}
                  onChange={(e) =>
                    updateSettings(profileId, {
                      memory: { ...settings.memory, utility_api_preset_id: e.target.value }
                    })
                  }
                  style={{ width: '100%' }}
                >
                ...
                <input
                  ...
                  min={1}
                  value={
                    settings.memory?.collections?.find((c) => c.id === 'events')?.retrieval
                      ?.count ?? 5
                  }
                  onChange={(e) => {
                    const n = Math.max(1, Number(e.target.value) || 1)
                    const collections = (settings.memory?.collections ?? []).map((c) =>
                      c.id === 'events' ? { ...c, retrieval: { ...c.retrieval, count: n } } : c
                    )
                    updateSettings(profileId, { memory: { ...settings.memory, collections } })
                  }}
                />
                ...
                <input
                  type="number"
                  min={1}
                  value={settings.memory?.checkpoint_turns ?? 6}
                  onChange={(e) =>
                    updateSettings(profileId, {
                      memory: {
                        ...settings.memory,
                        checkpoint_turns: Math.max(1, Number(e.target.value) || 1)
                      }
                    })
                  }
                />
