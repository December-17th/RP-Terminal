...
// NOTE: an earlier `prunedNodes(doc, inactiveEdges)` helper lived here — a static prune over a
// precomputed inactive-edge set. It was retired (nothing but its own test used it): pruning
// depends on which signals FIRE, which is only known during execution, so the engine interleaves
// pruning with its forward topo pass in workflowEngine.ts's runNodes and a static version would
// drift from those semantics (it lacked signal-gating).
