    ...
    return rulesArray.map((rule: any) => {
      ...
      try {
        return {
          ...
          placement: rule.placement ? (Array.isArray(rule.placement) ? rule.placement : [rule.placement]) : ['text'],
...
export const applyRegexRules = (text: string, rules: StRegexRule[], targetPlacement: string = 'text'): string => {
  let result = text;
