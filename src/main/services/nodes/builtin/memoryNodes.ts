...
import {
  composeMaintainerMessages,
  memoryMaintainConfig,
  type MemoryMaintainConfig
} from '../../memory/maintainerCompose'
import {
  chatTemplate,
  applyTableEdit,
  dueTables,
...
// `composeMaintainerMessages` and `memoryMaintainConfig` moved to `services/memory/maintainerCompose.ts`
// (execution-plan M5b) and are imported above; the node run below and the preview/Agent share that one
// composer. Re-exported so any node-internal importer keeps resolving them from here unchanged.
export { composeMaintainerMessages, memoryMaintainConfig }
