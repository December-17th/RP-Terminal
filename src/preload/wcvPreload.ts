...
// --- Card-UI layout compat: un-squash collapsed flex-column children ---
// Some card UIs are authored for SillyTavern's AUTO-HEIGHT iframe and lay content out in FIXED-height flex
// columns (a scroll region). Those columns' children commonly carry `flex-basis:0` and/or a computed
// `min-height:0`, so inside our FIXED-size WebContentsView the flex algorithm collapses each child BELOW
// its content — clipping/overlapping the text (the "custom-start UI squashed to one line" report). In ST
// the wrapping iframe grows to content, so the columns are never over-constrained and the collapse never
// fires; only our fixed frame triggers it, and the UI is CDN-hosted so it can't be patched card-side.
//
// Fix (verified in an Electron/Chromium harness): pin a content-height FLOOR on any flex-column child whose
// content overflows its box. `min-height: fit-content` is width-aware (text still wraps) and beats a 0
// flex-basis — `flex-shrink:0` alone does NOT (the item stays at its 0 basis). We only ever GROW a squashed
// item and we skip real scroll containers (`overflow-y: auto|scroll`), so intended layouts are untouched.
// Runs once the card has mounted and re-runs (debounced) as its DOM / the viewport changes.
;(function installUnsquashCompat(): void {
  ...
  const pass = (): void => {
    ...
    let scrollRegions = 0
    let hits = 0
    for (const el of Array.from(body.querySelectorAll('*')) as Element[]) {
      scanned++
      if (floored.has(el)) continue
      const p = el.parentElement
      if (!p) continue
      const pcs = w.getComputedStyle(p)
      if (!pcs || pcs.display.indexOf('flex') === -1) continue
      if (pcs.flexDirection.indexOf('column') !== 0) continue // column / column-reverse only
      colChildren++
      const oy = w.getComputedStyle(el).overflowY
      if (oy === 'auto' || oy === 'scroll') {
        scrollRegions++
        continue // a real scroll region owns its own height
      }
      if (el.scrollHeight > el.clientHeight + 2) {
        if (DEBUG)
          console.info(
            `[rpt-unsquash] floor <${el.tagName.toLowerCase()} class="${el.className}"> ` +
    ...
    if (DEBUG)
      console.info(
        `[rpt-unsquash] pass: scanned=${scanned} flexColChildren=${colChildren} ` +
          `skippedScrollRegions=${scrollRegions} floored=${hits} (total ${flooredTotal})`
      )
  }
  ...
  const schedule = (): void => {
    ...
    timer = setTimeout(() => {
      ...
      try {
        ...
        if (DEBUG) console.warn('[rpt-unsquash]', e)
      }
    }, 120)
  }
  const start = (): void => {
    schedule()
    try {
      // Re-run as the SPA renders/navigates (childList only, so our own inline-style writes don't re-trigger)
      // and as the frame is resized (which is exactly when a fixed-height column becomes over-constrained).
      new MutationObserver(schedule).observe(w.document.body, { childList: true, subtree: true })
      new ResizeObserver(schedule).observe(w.document.documentElement)
      // Web-font swap can grow content past its box without any childList/resize event — re-check once.
      if (w.document.fonts && w.document.fonts.ready) w.document.fonts.ready.then(schedule, () => {})
    } catch (e) {
      if (DEBUG) console.warn('[rpt-unsquash] observers', e)
    }
  }
  if (w.document && w.document.readyState !== 'loading') start()
