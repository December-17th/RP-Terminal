...
| 2.5 | `{{roll::0}}` (zero-sided)                                                                                        | `0`→`1d0`; droll rejects `d0` → warns, returns `''`.                        | Same result (`''`) — but note this diverges from RPT's _pre-issue-13_ behavior, which returned `'0'`. Deliberate: matches ST.                                 | ST `core-macros.js:322-331` + `droll.js:62`; RPT `src/shared/macros.ts` `roll`.                                   |
...
injection can appear _below_ the trailing user turn.
...
- **RPT:** keeps a trailing **user action last** (the L4-last invariant, so the live input is always the
  final message the model sees). A depth-0 injection lands just _before_ that trailing user action instead.
...
- **Grounding:** ST `public/scripts/openai.js:3827-3866` + `:1599-1601`; RPT
  `src/main/services/promptMessageShaping.ts` (`squashSystemMessages`, `mergeConsecutiveRoles`),
...
| 6.8 | `RegexBinding.activationOrder` permutation                   | User-sortable tier order (codes 0=global 1=character 2=preset; default `[2,0,1]` = preset→global→character), so a non-default permutation reorders which tier's regex wins. | **Parsed but has zero consumers.** The runtime projection collapses RegexBinding to a single boolean (`regexBindingEnabled`); assembly uses a FIXED tier order (`SPRESET_REGEX_TIER_ORDER`, preset→global→world→session). A non-default `activationOrder` is silently discarded — only "on ⇒ preset-first" is honored. | spec §RegexBinding `activationOrder` (spec:1528-1530); RPT `src/shared/spreset.ts` (`activationOrder` schema field, `SPresetProjectionSchema.regexBindingEnabled`, the fixed `SPRESET_REGEX_TIER_ORDER`). |
...
- **RPT:** clamps the injection index into the conversation region
  (`index = max(start, min(base − depth, maximumIndex))`, `promptDepthInjections.ts`), so a depth > MAX is injected at the
...
- **Grounding:** ST `public/scripts/openai.js:801-864` (loop + splice + final reverse), `:813` (`=== i`
  filter), `script.js:499` + `:3222-3223`; RPT `src/main/services/promptDepthInjections.ts` (depth splice-plan
...
mutate the wire in ways that are not part of ST's assembled prompt (ADR 0016 pins _assembly_ parity):
...
BOTH sides — the difference is now WHERE, not WHETHER (this was a _presence_ divergence before the owner
...
- **Absent vs. DISABLED marker:** the safety net fires only when the `charDescription` marker is _fully
  absent_ from the imported order. A marker the author included but explicitly DISABLED (`enabled:false`) is
