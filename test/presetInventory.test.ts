...
describe('hasRemoteCodeLoad (pure remote-code detector)', () => {
  ...
  it('does NOT flag local code, bare-specifier imports, or plain data fetches', () => {
    ...
    // Owner decision (ADR 0017): this detector is an evadable label, not a security boundary.
    expect(hasRemoteCodeLoad('fetch(url).then(r => r.text()).then(eval)')).toBe(false)
