...
**Status:** **Partially superseded / absorbed — Implemented (v1) 2026-07-11** (branch
`feat/plot-recall`). This doc's **`memory.recall`** node was **upgraded** into the catalogue/planner
recall described in [plot-recall-memory-design.md](plot-recall-memory-design.md) (its `entries` output
was dropped, its `<Query>` grep folded into the larger recall side-call) — for recall, that doc wins.
The rest of this design **shipped as specified**: the `notesGrep.ts` pure engine
([`src/shared/memory/notesGrep.ts`](../src/shared/memory/notesGrep.ts)), the per-chat notes file +
service ([`notesMemoryService.ts`](../src/main/services/notesMemoryService.ts)), and the
**`notes.maintain`** node ([`notesNodes.ts`](../src/main/services/nodes/builtin/notesNodes.ts),
`<MemoryNote section= mode=>` parse → `mergeNotes` → `writeNotes`). Two deltas from the plan below:
the opt-in `settings.notesMemory` flag was **dropped** (opt-in = wiring the node into a workflow; an
unwired node is inert), and the notes pack was **not** shipped (the ship vehicle is the
`plot-recall.rptflow` example workflow, per the pack-system-removal finding). See the other doc's
[§As-built (v1)](plot-recall-memory-design.md#as-built-v1--deviations) for the full deviation list.
**Date:** 2026-07-10
