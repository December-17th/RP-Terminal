...
| **World Assets**       | `assetUrl(name, type?, mood?)`; `assetList(name, type)`; `rptHost.requestAssetImport({name, type, variant?})` | ✅ | Local assets use `<name>_<type>[_<变体>].<ext>`. Character types are `头像`, transparent/compositable `立绘`, background-bearing `立绘bg`, and `相册`; scene types are `背景`/`全景`; cutscenes use `CG`. PNG/JPEG/WebP/GIF are supported for every type. MP4 is supported only for `立绘bg`, `背景`, `全景`, and `CG`, with byte-range streaming. `assetUrl` with `type` **omitted** resolves through a chain `立绘` → `立绘bg` → `头像` (first non-null wins, `thRuntime/index.ts:651-657`); explicit typed lookups remain strict (single lookup). Both transports share the same runtime contract and `rptasset://` serving path. |
...
**World Assets on WCV cards**: `rptasset://` is registered on `persist:wcv-cards`, including media range requests. `window.assetUrl(name, type = '立绘', mood?)` routes `头像`/`立绘`/`立绘bg`/`相册` to `character`, `背景`/`全景` to `location`, and `CG` to `cg`. WCV also exposes `assetList` and picker-backed `rptHost.requestAssetImport` at parity with inline cards.
