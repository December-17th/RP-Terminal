...
| Chat-completion **preset**                                        | preset JSON                                              | [`stPresetParser`](../../src/main/parsers/stPresetParser.ts) → preset files + `rp_terminal.presets`. Markers map to ST 1.18.0 parity (issue 11 / ADR 0016): distinct `world_info_before`/`world_info_after`, `char_personality`, `scenario` (each keeps its own role/position); `injection_trigger` (generation-type allow-list) and `forbid_overrides` carried through; card `system_prompt`/`post_history_instructions` override `main`/`jailbreak` at build time; duplicate identifiers resolve first-match. Native RPT presets keep the single `world_info` marker + folded description. | ✅ parser; 🟡 bundle import                                                                                                         |
