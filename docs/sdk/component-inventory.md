...
Every claim below cites the file it was verified against (per `AGENTS.md` grounding). When you change one
...
**Whole-namespace `misc` enumeration** (M3): `miscAssets()` takes **no arguments** and returns the card's
entire general-purpose card-art namespace — `{ name, variant, url, remote }[]`, local entries first then
remote — for duel card faces, item icons, skill art and panel ornaments. See §3's `misc` section for the
filename convention, the `variables.rpt_misc_assets` bag, and the ordering rules. Behavior lives once in
[`shared/thRuntime`](../../src/shared/thRuntime/index.ts) →
[`Host.miscAssets`](../../src/shared/thRuntime/hostFacets.ts); both transports inherit, and both main-side
entry points (`asset-misc-for-card` / `wcv-host-misc-assets`) call the single shared body
[`worldAssetService.miscAssetsForWorld`](../../src/main/services/worldAssetService.ts). Parity is pinned by
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Variables / MVU**    | `getVariables`, `insertOrAssignVariables`, `insertVariables` (no-overwrite), `replaceVariables`, `updateVariablesWith`; `Mvu.*`                                                                                              | ✅      | State of truth = `floor.variables.stat_data`. Default writes → RFC-6902 JSON-Patch (`applyVariableOps`). Variable merge/insert helpers honor `type:'script'` (per-card KV), `type:'chat'` (per-chat card KV), and `type:'global'` (per-profile KV). `type:'chat'` **reads** additionally layer that KV over the floor's local/template variables (`floors.variables` minus `stat_data`/`delta_data`, via `Host.getFloorVars`) — upstream both are `chat_metadata.variables`, so a lorebook `setLocalVar` is visible to a chat-scope read; **writes still target only the per-chat KV**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Prompt injection**   | `injectPrompts`, `uninjectPrompts`                                                                                                                                                                                           | 🟡      | Safe **no-op** (returns `{ uninject }`). Prompt is built in main; renderer-side injection can't reach it yet — cards calling these per-turn no longer throw. A separate main-side **pre-dispatch mutation seam** (issue 19 / the `CHAT_COMPLETION_PROMPT_READY` analogue, `generation/dispatchHooks.ts` + `promptArtifact.applyDispatchTransforms`) rewrites the FINAL message array at the 18e dispatch boundary; each real change is `opaque`-recorded (script id + hook + before/after hashes). Live cross-realm hook wiring + exact TH event name/payload mutability are **F2/F3-guarded** (docs-silent).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **Agent scheduling**   | `rpt.agents.run`, `runPlan`, `registerTool`, `onFloorCommitted`                                                                                                                                                              | 🟡 held | **Implemented in code at both transports but NOT a shipped card contract** — the owner decision is to hold it dormant (`execution-plan-2026-07-19` §3 D2), so a card must not rely on it yet. When shipped: RPT-only card-owned background scheduling — direct JSON input; authoritative profile/chat/card scope; abortable runs/plans and tool callbacks; staged tool operations; exact-once new-floor event with current/previous variables; same-Agent same-floor coalescing; no RPT variable/time scheduler. Successful text results fold native `<UpdateVariable>` / `<JSONPatch>` updates onto the current latest floor before their Run Record is finalized; the raw result remains unchanged. **Cards may NOT choose the API preset or model** — `apiPresetId`/`model` are omitted from `CardAgentRunOptions` and stripped main-side ([`AgentHostSession.ts`](../../src/main/services/agentRuntime/AgentHostSession.ts)); a card-invoked run instead honors the **user's per-Agent preset binding** (`invocationConfig.apiPresetId`), parity with the manual/trigger paths, and a slot reused for a new chat/profile rebinds to a fresh scoped session. Inline and WCV share the realm-neutral [`agentHostFacet.ts`](../../src/shared/thRuntime/agentHostFacet.ts); main authority and lifecycle share [`AgentHostSession.ts`](../../src/main/services/agentRuntime/AgentHostSession.ts). Transport adapters retain only scope and wire delivery (`cardBridge/host.ts`, `wcvHost.ts`, `agentRunIpc.ts`, `wcvIpc.ts`; ADR 0022). See [rpt-api.md](../rpt-api.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **World Assets**       | `assetUrl(name, type?, mood?)`; `assetList(name, type)`; `miscAssets()`; `rptHost.requestAssetImport({name, type, variant?})` | ✅ | Local assets use `<name>_<type>[_<变体>].<ext>`. Character types are `头像`, transparent/compositable `立绘`, background-bearing `立绘bg`, and `相册`; scene types are `背景`/`全景`; cutscenes use `CG`; general-purpose card art (card faces, item/skill icons, ornaments) uses `misc`, enumerable in full via `miscAssets()`. PNG/JPEG/WebP/GIF are supported for every type. MP4 is supported only for `立绘bg`, `背景`, `全景`, `CG`, and `misc`, with byte-range streaming. `assetUrl` with `type` **omitted** resolves through a chain `立绘` → `立绘bg` → `头像` (first non-null wins, `thRuntime/index.ts:651-657`); explicit typed lookups remain strict (single lookup). Both transports share the same runtime contract and `rptasset://` serving path. |
...
| **Display / transcript** | `renderFloors(from, to)` → `Promise<RenderedFloorView[]>`; `displayRevision()` → `number` (sync); `setDisplayStreamEnabled(enabled)` → `Promise<void>`                                                                       | ✅ (WCV only) | **RPT-only** (ADR 0023). The app's own view-time display transform (EJS → markers → macros → display regex), for a card that OWNS the chat rect. **Trusted WCV cartridge panels only, fail-closed** — inline (non-WCV) cards get **inert stubs by design** (v1 non-goal; `cardBridge/host.ts:505-511`). Returned `html` is **RAW beautified HTML** (no sanitization tier — trusted-panel model). `renderFloors` renders committed floors (inclusive, `floors()` indexing) — **batch clamped to 32 floors** main-side ([`displayRenderBroker.ts`](../../src/main/ipc/displayRenderBroker.ts)`:30,55`), forwarded to the renderer (transform is renderer-only), **10 s timeout → `[]`** + fail-closed `[]` on unresolved session (`wcvIpc.ts:1087-1097`); the renderer holds an **LRU(64)** cache keyed by `(chatId, floorIndex, swipeId, revision, marker fingerprint, content stamp)` ([`displayBroker.ts`](../../src/renderer/src/display/displayBroker.ts)`:153,304`) — the content stamp hashes `response.content` + `variables` + `plot_block`, invalidating on message edits / floor deletes / MVU var writes. `displayRevision()` is sync (main-cached, fallback 0) and **bumps on regex / settings-flag / character (id, name, reasoning_template) / persona** changes. `setDisplayStreamEnabled` opts a panel in/out of transformed streaming; frames cost nothing when no panel opted in. **Events ride the generic `wcv-event`** (no new channels), delivered **per-CHAT** to opted-in panels: `display_stream_frame: DisplayStreamFrame` at the native `rateChars` checkpoint cadence, `display_invalidated: { revision, reason: 'regex'\|'settings'\|'character'\|'persona' }`. Opt-ins tracked per sender webContents, deduped chat set relayed to the renderer; on renderer startup a `display-broker-ready` handshake makes main re-relay the enabled-chat set + send `display-revision-seed` so a main-window reload keeps stream opt-ins and never rewinds the revision (`wcvIpc.ts:208-227, 1120-1126`). Facet [`DisplayHost`](../../src/shared/thRuntime/hostFacets.ts)`:193-203`; wire shapes [`displayView.ts`](../../src/shared/thRuntime/displayView.ts); channels [`wcvChannelSpec.ts`](../../src/shared/thRuntime/wcvChannelSpec.ts)`:138-140`. Bare card globals via [`thRuntime/index.ts`](../../src/shared/thRuntime/index.ts) + mirrored on `rptHost.*` on WCV panels ([`wcvPreload.ts`](../../src/preload/wcvPreload.ts), like `requestOverlay`). **Segmentation companions** (PURE, transport-independent — no Host/IPC): `splitDisplayHtml(html)` → `Segment[]`, `isInteractiveHtml(html)` → `boolean`, `applyScriptedHtml(el, html)` route the RAW `renderFloors` html the same way the native transcript does, so a panel need not reimplement `MessageContent` ([`displayBlocks.ts`](../../src/shared/displayBlocks.ts)). See [rpt-api.md §Display](../rpt-api.md). |
...
| **App-owned surfaces** | `openFloorManager()` → `void` | ✅ | **RPT-only.** Open the app's floor manager (楼层管理) — the floor list + delete-tail modal the native chat toolbar's `☰` raises ([FloorManagerModal.tsx](../../src/renderer/src/components/FloorManagerModal.tsx)). Unlike `requestOverlay`, the **app** owns and renders the surface: the card names no `entry` and supplies no DOM. It exists because a card that owns the play area via `panel_ui` never mounts the native chat UI and so loses the toolbar route to it. **Fire-and-forget** (no return value, no opened/failed signal), scoped to the **active chat** (dropped otherwise), and there is no card-side close — the user dismisses via ✕ / Esc / backdrop. Mounted from [App.tsx](../../src/renderer/src/App.tsx) beside the Duel / Assets / Memory popups so it layers above both the Workspace and a card's `panel_ui`; state in `uiStore.floorManagerOpen`. Both transports via [`Host.openFloorManager`](../../src/shared/thRuntime/hostFacets.ts); WCV also exposes `rptHost.openFloorManager`. See [rpt-api.md §App-owned surfaces](../rpt-api.md). |
...
- Read: `getVariables({ type: 'chat' })` → arbitrary JSON object. **The read is a MERGE**, upstream-faithful:
  the floor's local/template variables as the base, the per-chat KV layered on top —
  `{ ...floors.variables (minus stat_data/delta_data), ...chatCardKV }`. In SillyTavern both are one bag
  (`chat_metadata.variables`), so a lorebook/preset `setvar` / `setLocalVar('char_info_visuals', …)` is
  readable here; RPT keeps two stores and merges on read. On a key collision the **card's own KV wins**.
...
- **Writes are per-chat-KV only.** Every `{type:'chat'}` write path (`replaceVariables`,
  `updateVariablesWith`, `insertVariables`, `insertOrAssignVariables`, `deleteVariable`) reads AND writes
  the per-chat KV alone — a read-modify-write never persists a floor local variable into the KV, and a card
  cannot write a template local var through this scope.
...
- **NOT MVU `stat_data`:** not AI-authored, not sent to the model, not validated/stripped by the card's
  `data_schema`. `stat_data`/`delta_data` are excluded from the chat-scope read (they stay message-scope,
  reachable via `getVariables()` with no option). Use `type:'chat'` for UI/session state; use `stat_data`
  for story state.
- Backed by `chatCardVarsService` (`profiles/<profileId>/chats/<chatId>/session-vars.json`), exposed via
  the `Host` (`getChatVars`/`setChatVars`, plus `getFloorVars` for the merged base) and both transports.
  The stat_data/delta_data omission is the one shared pure helper
  [`shapes.floorLocalVars`](../../src/shared/thRuntime/shapes.ts), used by inline and WCV alike.
...
**World Assets on WCV cards**: `rptasset://` is registered on `persist:wcv-cards`, including media range requests. `window.assetUrl(name, type = '立绘', mood?)` routes `头像`/`立绘`/`立绘bg`/`相册` to `character`, `背景`/`全景` to `location`, `CG` to `cg`, and `misc` to `misc` ([`categoryForType`](../../src/shared/worldAssets/types.ts)). WCV also exposes `assetList`, `miscAssets` and picker-backed `rptHost.requestAssetImport` at parity with inline cards.

**`misc` — general-purpose card art (M3)**: the asset category for artwork that is **not** a character
portrait, a location backdrop, or a story CG — duel card faces, item icons, skill art, panel ornaments. It
is a first-class `AssetCategory` **and** `AssetType`, so nothing about the character namespace changes.

_What a card author does_ — **local files** go in the world's `misc` folder under the standard
`<name>_<type>[_<变体>].<ext>` convention with `misc` as the type token:

| file                   | `name` | `variant` |
| ---------------------- | ------ | --------- |
| `火球术_misc.png`      | 火球术 | `null`    |
| `火球术_misc_alt.webp` | 火球术 | `alt`     |
| `火球术_misc_2.webp`   | 火球术 | `2`       |

png/jpg/jpeg/jpe/webp/gif are accepted, and **mp4 too** — `misc` is full-frame art like `CG`
([`VIDEO_ASSET_TYPES`](../../src/shared/worldAssets/types.ts):31). **Remote files** are declared by writing
a bag of HTTPS URLs into the newest persisted floor's variables (the same mechanism as
`char_info_visuals`, a **separate** namespace):

```js
variables.rpt_misc_assets = {
  火球术: { url: 'https://cdn.example/fireball.png' },
  陨星裂空: { url: 'https://cdn.example/meteor.mp4' }
}
```

HTTPS only, no credentials in the URL; anything else is silently dropped
([`remote.ts`](../../src/shared/worldAssets/remote.ts):45-53). Remote entries are proxied as
`rptremoteasset://misc/<profile>/<chat>/<name>` — a **different host segment** from the character shim's
`rptremoteasset://asset/…`, so the same name may live in both bags without either shadowing the other
([`remote.ts`](../../src/shared/worldAssets/remote.ts):23-26,
[`remoteAssetService.ts`](../../src/main/services/remoteAssetService.ts):69).

_Reading them_ — `await miscAssets()` returns the **whole** namespace, `{ name, variant, url, remote }[]`;
the app does no matching, the card UI parses the list. All local entries come first (so taking the first
match per name gets the local file), then remote; names natural-sorted, base (`variant: null`) before
variants, remote entries always `variant: null`. Both transports call the one shared body
[`worldAssetService.miscAssetsForWorld`](../../src/main/services/worldAssetService.ts):561-606 — WCV via
...
card art no longer has to borrow the character namespace.

**Remote `char_info_visuals` compatibility**: the newest persisted floor is the source of truth. A valid
HTTPS `variables.char_info_visuals[name].url` is exposed as read-only `立绘bg` through
`rptremoteasset://asset/<profile>/<chat>/<name>`; the source URL is resolved main-side on every request and
is not embedded in the protocol URL. Local `立绘bg` wins, while explicit `立绘` and non-character
lookups remain strict. The handler streams supported images/GIF/MP4 without disk writes and forwards a
single byte range for MP4 playback. The default and `persist:wcv-cards` sessions both register the scheme,
and the shared fallback helper keeps inline/WCV behavior aligned. Verified in
...
| `pin_paths` (`string[]`)             | **Context pins (WP-L3).** Dot-paths into the latest-floor variables (e.g. `stat_data.location`, `stat_data.party`) whose current values are appended to the lore **scan text** so state-relevant keyed entries keep firing after recent messages stop naming them. Resolution + block format in [`buildPinBlock`](../../src/main/services/promptBuilder.ts); wired at [`genContext.ts`](../../src/main/services/generation/genContext.ts). Scalars stringify; a scalar array of ≤8 elements joins with `, `; objects, longer/non-scalar arrays, and missing paths contribute nothing (dev-diagnostic, never guessed). Block is `\n[PINS] <label>: <value> \| …` (label = last path segment, order = config order). **Matcher input ONLY — pin values never enter the model prompt.** **APP-GATED: requires `settings.lorebook.context_pins` (default OFF).** Declaring `pin_paths` is a request, not a switch — a card cannot widen what the matcher sees on its own; with the gate off the scan text is byte-identical to the ST keyword baseline. The Debug window's Retrieval tab previews what pins *would* fire regardless of the gate. | 🟡 opt-in                                                                                                                                                                   |
...
| **World Card bundle slots**          | `world_card` (version marker), `meta`, `regex[]`, `presets[]`, `lorebooks[]`, strict `agents[]`, optional `agent_role_recommendations`, legacy-readable `workflows[]`, `table_templates[]`, `plugins[]`, `agent`, `combat`, `recommended_settings`                                                                                                                                                                                                                       | ✅ schema; routing varies by slot (see §5)                                                                                                                           |
| `agents[]`                           | strict `rpt-agent` Agent Definitions installed enabled into the profile-wide Agent Catalog; `agent_role_recommendations` may name a compatible Classic Narrator or Yuzu Scene Director but does not override the player's binding. **No API preset on import (owner policy):** an imported Agent is installed with NO API preset/model applied — any declared `apiPresetId`/`model` is stripped at install ([`AgentCatalog.neutralizeImportedPreset`](../../src/main/services/agentRuntime/catalog/AgentCatalog.ts)), the model kept only as the display-only `modelHint` recommendation; the user must pick a preset (`invocationConfig.apiPresetId`) or the run falls back to the profile's active preset. Other overrides (`generationParameters`, bundled preset, retries) are kept.                                                                                                                                                                                                                                    | ✅ contract + card install bridge ([`character.ts`](../../src/main/types/character.ts), [`cardAgentCatalogBridge.ts`](../../src/main/services/cardAgentCatalogBridge.ts)) |
| `workflows[]`                        | **Removed.** Legacy workflow docs / `rpt-workflow-bundle` envelopes are no longer imported or routed; the slot is preserved as inert unknown-passthrough only                                                                                                                                                                                                                                                                                                            | 🗑 Removed on `agent-system` (ADR 0020); superseded by `agents[]`                                                                                                    |
...
hash is the source version, so a re-scan is idempotent and an edited file is an _upgrade_ (with an
edit-conflict prompt when the same field was changed in-app). Verify:
[`agentFolder.ts`](../../src/main/services/agentRuntime/catalog/agentFolder.ts) (`AGENT_FILE_EXTENSION`,
...
The Agent Runtime cutover ([ADR 0020](../adr/0020-agent-runtime-replaces-workflow-system.md)) has
landed on `agent-system`: the card `workflows[]` import contract was **removed**, not migrated. Cards
now carry Agent Definitions in `agents[]` instead. The Workflow row below is retained for history and
marked removed.

| ST / TH card element                                              | Lives in (ST)                                            | RPT destination                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Status                                                                                                                                                                                                     |
| ----------------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Core character fields                                             | `data.*`                                                 | `CardDataSchema` (`data.*`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | ✅ direct                                                                                                                                                                                                  |
| Embedded lorebook                                                 | `data.character_book`                                    | lorebook library at `id == characterId` ([`LorebookSchema`](../../src/main/types/character.ts)). Import (`normalizeLorebookData`) is **non-destructive**: the source entry `uid`/`id` is preserved as the entry `id` (minted once on first save by `saveLorebookById`, the single authority); the source `extensions` object is merged into the opaque `extra` bag; unconsumed source fields (`sticky`, `cooldown`, `delay`, `group`/`groupOverride`/`groupWeight`/`useGroupScoring`, `selectiveLogic`, `matchWholeWords`, `scanDepth`, `position`, `depth`, `role`, `vectorized`, `automationId`) ride along verbatim under `extra.st_source` for a future timed/groups WP and re-export. Matching/placement behavior is unchanged (WP-L1). | ✅                                                                                                                                                                                                         |
| Standalone world info                                             | separate JSON                                            | lorebook library (uuid id) — same `normalizeLorebookData` preservation of `id` + `extensions` + `extra.st_source` as embedded (WP-L1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | ✅                                                                                                                                                                                                         |
...
| Workflow doc / bundle                                             | RPT workflow JSON                                        | **Removed.** `rp_terminal.workflows[]` is no longer imported or routed; the key is preserved as inert unknown-passthrough in the card schema but reaches no runtime ([`character.ts`](../../src/main/types/character.ts))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 🗑 Removed on `agent-system` (ADR 0020); superseded by Agent Definitions (`agents[]`)                                                                                                                      |
| Agent Definition                                                  | `rpt-agent` JSON                                         | `rp_terminal.agents[]` → profile Agent Catalog, enabled on initial card install; name collisions require an incoming rename through the Catalog Interface, and card re-import does not silently upgrade pinned baselines                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | ✅ strict contract + catalog install ([`AgentCatalog.ts`](../../src/main/services/agentRuntime/catalog/AgentCatalog.ts), [`cardAgentCatalogBridge.ts`](../../src/main/services/cardAgentCatalogBridge.ts)) |
...
**Lorebook key matching** (`lorebookService.entryQualifies`): a key fires as a substring hit on the
scan text, honoring `case_sensitive` by lowercasing both sides. A key given in ST **regex form** —
slash-delimited `/pattern/flags`, flags drawn from `g i m s u y`, internal `/` escaped — is compiled
and tested against the **untransformed** scan text, so the regex's own `i` flag governs case and
`case_sensitive` does not apply to it; the CJK card ecosystem relies on this. A key that looks
slash-delimited but fails to compile falls back to plain literal matching of the whole key string
(slashes included). This applies identically to primary `keys` and `secondary_keys`, and to recursion
passes (WP-L2). **Recorded deviation:** RPT does substring matching only — it behaves as ST
`matchWholeWords: off` (no word-boundary option), which is what CJK cards need.

**Context pins** (WP-L3, `pin_paths` above) are **off unless the app owner enables them**
(`settings.lorebook.context_pins`; Settings → the lorebook block). With the gate off — the default —
retrieval is exactly ST keyword matching and a card's `pin_paths` has no effect. With it on, pins
extend the **scan text** the matcher sees: the card
declares dot-paths into the latest-floor variables, and `buildPinBlock` appends their current values
as a delimited `\n[PINS] …` line in `buildGenContext` (`genContext.ts`). Pinned values therefore
participate in the first pass and recursion exactly like conversation text, so a keyed entry fires
when the *state* is relevant even after recent messages stop naming the keyword. The block is
**scan-only** — it is never threaded into the assembled prompt.
...
world-owned regex, scripts, and cartridge code; **import as new** creates an independent copy;
**replace** fully imports the new copy before deleting the old world and its sessions. Updates do not
reinstall extra lorebooks or table templates because those library artifacts have no world ownership and
would otherwise duplicate under new ids. `buildWorldCardExport` writes the existing inverse
card/lore/regex surface. Remaining routing (plugins/scope) is tracked in
[world-card-design.md](../world-card-design.md) §5/§9.
