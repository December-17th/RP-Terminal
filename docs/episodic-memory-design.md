...
**T4 — Vector backend. ✅ RESOLVED: brute-force cosine in JS.** Embeddings live in an `embedding` column (JSON float array); recall computes cosine in JS over a chat's memories (fine to a few thousand rows — one chat rarely exceeds that), no native dependency. `sqlite-vec` stays a future optimization if a chat's memory ever gets large. **Hybrid (keyword + vector, reciprocal-rank fusion) > pure vector** for RP — keyword anchors to the proper nouns the player typed. Vector/hybrid activate only when an `embedding_api_preset_id` is configured; otherwise recall silently stays keyword. _(Rejected for v1: sqlite-vec — adds a per-platform native binary.)_
