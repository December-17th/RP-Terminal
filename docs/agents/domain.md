...
(If this repo ever splits into genuinely separate domains — e.g. the card runtime vs. the combat
engine as independent glossaries — re-run `/setup-matt-pocock-skills` and switch to multi-context,
which adds a root `CONTEXT-MAP.md` pointing at per-context `CONTEXT.md` files.)
