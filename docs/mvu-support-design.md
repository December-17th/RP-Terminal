...
| **R2 — Schema + init** | MVU state schema (native JSON-described first) + defaults; `[initvar]` lorebook seeding; validate/reconcile `stat_data` after folds; add a `chat` variable scope. | Zod 4, `chats` migration pattern |
| **R3 — RPG UI upgrade** | Nested/recursive widgets (objects, arrays, value+description), schema-derived auto-layout, live `mag_*` updates, tokenized styles. **The right-panel RPG UI.** | `LayoutRenderer`/`WidgetRegistry`, `setLatestFloorVariables` |
