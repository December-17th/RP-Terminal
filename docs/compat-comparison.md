...
| **Slash / STScript** `triggerSlash` (pipes, closures)                          | ✅                    | 🟡 subset (`shared/stscript`): `/gen`·`/genraw`·`/trigger`·`/send`·`/setinput`, chat+global vars; `/send` STAGES its text and the next `/trigger` generates with it (the 行动选项 clickable-options combo — `shared/thRuntime/index.ts` triggerSlash fallback); `while`/long-tail ⬜ |
