...
| **Slash / STScript** `triggerSlash` (pipes, closures)                          | ✅                    | 🟡 subset (`shared/stscript`): `/gen`·`/genraw`·`/trigger`·`/send`·`/setinput`, chat+global vars; `/setinput` and `/send` both fill the input box, and `/trigger` **presses the send button** (submits the current box content through the Composer's normal path — `Host.submitInput`, `shared/thRuntime/index.ts` triggerSlash fallback), so both 行动选项 clickable-options combos (`/send x \| /trigger`, `/setinput x \| /trigger`) work; a Host without `submitInput` falls back to an empty-action re-trigger; `while`/long-tail ⬜ |
| **Direct ST DOM pokes** `#send_textarea` / `#send_but` from message scripts    | ✅ (same page)        | 🟡 inline transport only: hidden compat stand-ins in the app document (`StDomCompat.tsx`) — `#send_textarea` mirrors the composer text (script `.value` writes + a dispatched `input` inject into the real box), `#send_but`/`#send_butt` click = press send. Inline message iframes are same-origin (`InlineCardFrame.tsx:188`) so `window.parent.document` reaches them; WCV cards are a separate WebContents and stay API-only |
...
- **Lorebook key matching:** ST regex keys (`/pattern/flags`, flags `dgimsuvy` — the full JS set)
  match; `case_sensitive`
  is ignored for a regex key (its own flags govern case), and an invalid regex-looking key falls back to
  literal matching **and logs why** (a slash-delimited key that fails to compile can never hit, so a
  silent fallback would be indistinguishable from a dead key) (WP-L2, `lorebookService.entryQualifies`).
  **Recorded deviation:** RPT matches keys
