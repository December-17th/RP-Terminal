...
compatibility with. Clean-room throughout — RPT reimplements the *surface/behavior* from public docs;
...
|---|---|---|---|
| **Kind** | A **JS script runtime** + frontend-card renderer, as a SillyTavern *extension* | A **prompt-template engine** (EJS) extending ST's macro syntax, as an *extension* | A **standalone Electron app** that is *format-compatible* with both |
...
RPT carries **both** compat surfaces: the EJS template engine *and* the TH JS API. The two extensions
...
lorebooks, regex, and presets. The compatibility *gaps* are the ST-PT template long-tail and the deeper
