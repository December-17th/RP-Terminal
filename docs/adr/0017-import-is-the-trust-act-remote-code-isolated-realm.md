...
- A character import can install bundled presets. If one is later activated, its non-remote TavernHelper scripts run in the isolated WCV under import trust even when the active card is untrusted; no card-consent prompt appears. This indirect card-supplied path is intentional under the owner-approved "import is the trust act" stance. Remote-code scripts still require the preset's separate high-trust opt-in.
