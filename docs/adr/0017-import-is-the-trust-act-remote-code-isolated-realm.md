...
- The `hasRemoteCodeLoad` classifier (`src/main/services/scriptService.ts`) enforces the opt-in conservatively: direct remote imports/URLs and fetch-to-text-to-executable-sink pipelines are remote code, including runtime-built and extensionless URLs. Ordinary data fetches are not remote code merely because they return text. Realm isolation remains defense in depth for arbitrary JavaScript that static analysis cannot prove.
