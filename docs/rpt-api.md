...
- `miscAssets()` → `Promise<Array<{ name: string; variant: string | null; url: string; remote: boolean }>>` (M3) — enumerate the card's **entire `misc` namespace**: general-purpose card art that is **not** a character portrait (`立绘`/`立绘bg`/`头像`/`相册`), a location backdrop (`背景`/`全景`), or a story CG — duel card faces, item icons, skill art, panel ornaments. **Takes no arguments and does no matching**: the app hands over the whole list and the card UI parses it. Also `window.miscAssets` / `window.TavernHelper.miscAssets`; standalone-preview guard: `typeof miscAssets === 'function'` before calling.
  ...
  **Ordering.** All **local** entries first, then **remote** — so a card taking the first match per name gets the local file, consistent with the local-first rule everywhere else. Names are naturally sorted (`localeCompare(…, 'zh', {numeric:true})`); within one local name the base (`variant: null`) comes first, then its variant tokens, same sort. Remote entries are always `variant: null` (the bag has no variant concept). Lorebook-id precedence mirrors `assetUrl`/`assetList` but is applied **per name**: the first world lorebook carrying a name wins, and entries are not merged across worlds. A name may legitimately appear twice (once local, once remote) — that is what makes "take the first match" work.
  ...
  `assetUrl(name, 'misc')` still works for a single lookup and falls back to the remote bag when no local file exists ([`REMOTE_FALLBACK_TYPES`](../src/shared/worldAssets/remote.ts):118). **`立绘bg` / `char_info_visuals` is unchanged and stays character-only** — `misc` exists so card art no longer has to borrow the character namespace. There is no Assets-view UI tab for `misc`; it is card-managed.
