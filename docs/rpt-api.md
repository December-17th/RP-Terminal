...
- **Persona macros** (ST-faithful): `{{user}}` = active persona **name**; `{{persona}}` = active persona **description** (empty when persona injection is off). Both transports resolve `{{persona}}` via the `personaDescription` host facet — inline (`cardBridge`) and WCV (`wcvPreload`) are at parity. The description is injected into the prompt (IN_PROMPT) at the preset's `personaDescription` marker position, falling back to a pre-conversation system block when the preset has no such marker.
