...
- **Chat scope** — `getVariables({type:'chat'})` / `insertOrAssignVariables(obj, {type:'chat'})` / `insertVariables(obj, {type:'chat'})` / `updateVariablesWith(fn, {type:'chat'})` / `replaceVariables(obj, {type:'chat'})` — ✅ (sync read) a per-chat, card-scoped KV store, **general scope for session UI/state** (e.g., adaptive-regex selections and the 命定之诗 party panel). Not in-prompt. **Namespace your keys** (e.g. `party.members`) to avoid collisions across multiple widgets in the same chat. **NOT `stat_data`** — use this for UI state, not story variables. Backed by `chatCardVarsService` (`profiles/<profileId>/chats/<chatId>/session-vars.json`), exposed via `Host.getChatVars`/`setChatVars`. **Reads also see the floor's local/template variables.** Upstream, ST's local variables live in `chat_metadata.variables` — the SAME bag TavernHelper's `type:'chat'` reads — so a lorebook/preset `setvar` / `setLocalVar('char_info_visuals', …)` is visible to a card reading chat scope. RPT stores the two separately, so `getVariables({type:'chat'})` returns `{ ...floorLocalVars, ...chatCardKV }`: the floor's top-level `floors.variables` as the BASE (minus `stat_data`/`delta_data`, which stay message-scope) with the per-chat KV layered ON TOP (the card's own value wins on a key collision). **Writes are unchanged** — `replaceVariables` / `updateVariablesWith` / `insertVariables` / `insertOrAssignVariables` / `deleteVariable` with `{type:'chat'}` read and write ONLY the per-chat KV, so a read-modify-write never copies a floor variable into `session-vars.json`, and a card cannot write a template local var. Exposed via `Host.getFloorVars` (`shared/thRuntime/hostFacets.ts`), merged in `shared/thRuntime/index.ts`; the key omission is the one shared helper `shapes.floorLocalVars`, used by both transports.
...
- **Card writes survive MVU re-evaluation (journaled).** `stat_data` is rebuilt from the model's `<UpdateVariable>` blocks on re-evaluate (`generationService.reevaluateVariables`, triggered by chat edits/deletes), and card/panel writes are **not** re-derivable from response text. So every card write is journaled per floor to the `floor_operations` table (FloorState) and **REPLAYED after that floor's model fold**: JSON-Patch writes (`applyVariableOps` — `Mvu.setMvuVariable` / `insertOrAssignVariables` / `mvu.set` node) as `kind:'patch'`, whole-replace (`replaceVariablesFromCard` — `Mvu.replaceMvuData` / `replaceVariables` via `wcv-host-set-vars`) as a `kind:'set'` on `variables.stat_data` — both with `source:'card'`. The **Variables-view whole-object debug write** (`setFloorStatData`) is journaled the same way with `source:'user'`. Floor truncation (regenerate/swipe/delete-from) rolls back the journal at/after the cut in the same transaction as the floors (`FloorState.deleteFromFloor`, reached via `chatService.truncateFloors`); chat deletion drops the whole per-chat session store. The **legacy `vars_ops` table** is retained read-only for pre-existing saves and imported lazily by `FloorState.pendingLegacy`. See `src/main/services/generation/varsWrite.ts` + `src/main/services/agentRuntime/floorState/FloorState.ts`.
...
- `assetUrl(name, type?, mood?)` → `Promise<string | null>` — resolve an asset (variant-aware) from the active world's asset layer. **When `type` is omitted**, the card-facing facade resolves through a chain — `立绘` → `立绘bg` → `头像`, first non-null wins (`thRuntime/index.ts:651-657`); an **explicit** `type` stays strict (single lookup, no fallback). Files follow `<name>_<type>[_<变体>].<ext>`; supported extensions are `.png`, `.jpg`, `.jpeg`, `.jpe`, `.webp`, `.gif`, and `.mp4`. `立绘` means compositable character art without a background; `立绘bg` means full-frame character art with a background. MP4 is accepted only for full-frame types (`立绘bg`, `背景`, `全景`, `CG`, `misc` — [`VIDEO_ASSET_TYPES`](../src/shared/worldAssets/types.ts):31); GIF remains an image and may use either standee type. A requested variant falls back to the base file. **The category is inferred from `type`** via [`categoryForType`](../src/shared/worldAssets/types.ts): `头像`/`立绘`/`立绘bg`/`相册` → `character`, `背景`/`全景` → `location`, `CG` → `cg`, and `misc` → `misc` (general-purpose card art — see `miscAssets()` below). Returns an `rptasset://` URL loadable in both card transports. Also exposed as `window.assetUrl` and `window.TavernHelper.assetUrl`.
...
- `miscAssets()` → `Promise<Array<{ name: string; variant: string | null; url: string; remote: boolean }>>` (M3) — enumerate the card's **entire `misc` namespace**: general-purpose card art that is **not** a character portrait (`立绘`/`立绘bg`/`头像`/`相册`), a location backdrop (`背景`/`全景`), or a story CG — duel card faces, item icons, skill art, panel ornaments. **Takes no arguments and does no matching**: the app hands over the whole list and the card UI parses it. Also `window.miscAssets` / `window.TavernHelper.miscAssets`; standalone-preview guard: `typeof miscAssets === 'function'` before calling.

  **Authoring — local files.** Drop them in the world's `misc` folder under the same convention as every other asset, with `misc` as the type token: `火球术_misc.png` → `{ name:'火球术', variant:null }`; `火球术_misc_alt.webp` → `{ name:'火球术', variant:'alt' }`; `火球术_misc_2.webp` → `{ name:'火球术', variant:'2' }`. png/jpg/jpeg/jpe/webp/gif, **and mp4** — `misc` is full-frame art like `CG` ([`VIDEO_ASSET_TYPES`](../src/shared/worldAssets/types.ts):31). Filenames parse through the ordinary [`parseAssetFilename`](../src/shared/worldAssets/filename.ts) because it anchors on the type token.

  **Authoring — remote files.** Write an HTTPS bag onto the newest persisted floor's variables — the same mechanism as `char_info_visuals`, but a **separate namespace**:
  ...
  HTTPS only, no credentials in the URL, entries failing that are silently dropped ([`remote.ts`](../src/shared/worldAssets/remote.ts):45-53); `.mp4` → video, everything else → image. Each is proxied on demand as `rptremoteasset://misc/<profile>/<chat>/<name>`, a **different host segment** from the character shim's `rptremoteasset://asset/…`, so the same name may exist in both bags without either shadowing the other ([`REMOTE_ASSET_URL_HOSTS`](../src/shared/worldAssets/remote.ts):23-26; the resolver's micro-cache key carries the kind, [`remoteAssetService.ts`](../src/main/services/remoteAssetService.ts):69). The source URL is resolved main-side per request and never embedded in the protocol URL.

  **Ordering.** All **local** entries first, then **remote** — so a card taking the first match per name gets the local file, consistent with the local-first rule everywhere else. Names are naturally sorted (`localeCompare(…, 'zh', {numeric:true})`); within one local name the base (`variant: null`) comes first, then its variant tokens, same sort. Remote entries are always `variant: null` (the bag has no variant concept). Lorebook-id precedence mirrors `assetUrl`/`assetList` but is applied **per name**: the first world lorebook carrying a name wins, and entries are not merged across worlds. A name may legitimately appear twice (once local, once remote) — that is what makes "take the first match" work.

  Returns `[]` on any miss or failure and **never throws into the card** (the guarantee lives in the shared runtime facade, so both transports degrade identically). Backed by [`Host.miscAssets`](../src/shared/thRuntime/hostFacets.ts) → the ONE main-side body [`worldAssetService.miscAssetsForWorld`](../src/main/services/worldAssetService.ts):561-606, which WCV (`wcv-host-misc-assets`, [`wcvIpc.ts`](../src/main/ipc/wcvIpc.ts):1065) and inline (`asset-misc-for-card`, [`worldAssetIpc.ts`](../src/main/ipc/worldAssetIpc.ts):116 → [`cardBridge/host.ts`](../src/renderer/src/cardBridge/host.ts):494) both call — parity by construction, pinned by `test/miscAssetsParity.test.ts`.

  `assetUrl(name, 'misc')` still works for a single lookup and falls back to **the `misc` bag** when no local file exists — the fallback carries the kind, so a `misc` lookup can never answer with a `char_info_visuals` portrait ([`remoteFallbackKindForType`](../src/shared/worldAssets/remote.ts):106-113, threaded through `localFirstRemoteAssetUrl`'s `resolveRemote(kind)` at both call sites: [`wcvIpc.ts`](../src/main/ipc/wcvIpc.ts):1034 and [`cardBridge/host.ts`](../src/renderer/src/cardBridge/host.ts):463). **`立绘bg` / `char_info_visuals` is unchanged and stays character-only** — `misc` exists so card art no longer has to borrow the character namespace. There is no Assets-view UI tab for `misc`; it is card-managed.
...
Distinct from the overlay pair above: these open a surface **the app owns and renders**, so the card names no `entry` and supplies no DOM. They exist because a card that owns the play area via `panel_ui` never mounts the native chat UI, and therefore loses the toolbar buttons that normally reach these surfaces.

- `openFloorManager()` → `void` — **RPT-only host method** (no vanilla-ST equivalent). Open RP Terminal's floor manager (楼层管理): the floor list + delete-tail surface, the same modal the native chat toolbar's `☰` button raises ([`FloorManagerModal.tsx`](../src/renderer/src/components/FloorManagerModal.tsx)). **Fire-and-forget** — it returns nothing and cannot report whether the modal opened. Scoped to the **active chat**: a request from a panel whose chat is not active is dropped. The modal is mounted by [`App.tsx`](../src/renderer/src/App.tsx) alongside the Duel / Assets / Memory popups, so it layers above **both** the reconfigurable Workspace and a card's static `panel_ui` grid. State lives in `uiStore.floorManagerOpen`; the user dismisses it by ✕ / Esc / backdrop, and there is no card-side close method. Also `window.openFloorManager` / `window.TavernHelper.openFloorManager`; on WCV panel surfaces additionally `window.rptHost.openFloorManager`. Both transports route to [`Host.openFloorManager`](../src/shared/thRuntime/hostFacets.ts) and both enforce the active-chat check — WCV over `wcv-host-open-floor-manager` (relayed on to the renderer as `wcv-host-floor-manager`), inline straight to the store.
