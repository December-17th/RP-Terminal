...
- `sceneAssetUrl(location, type)` → `Promise<string | null>` — 按层级地点解析`全景`或`背景`。文件名可以只写当前地点，也可选择性写入按原顺序出现的上级地点；如果当前地点无素材，解析器会使用最接近的可用上级地点背景。同分候选会安全失败，不会任意选择。该方法同时暴露为 `window.sceneAssetUrl` 和 `window.TavernHelper.sceneAssetUrl`；旧版主机会回退到精确 `assetUrl(location, type)`。
