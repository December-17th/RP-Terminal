...
- Global-variable **persistence** across calls (chat/local vars persist; `global` scope is ephemeral per run
  — see §3.4).
...
3. **Command → `Host` mapping (the `StCtx.fallback`).** Built-ins stay in the interpreter; everything else
   ...
   | --- | --- | --- |
   | `/setvar` `/getvar` `/addvar` (+ chat scope) | interpreter + `StCtx.setVar` → `host.applyVariableOps` | persisted chat var |
   | `/setglobalvar` `/getglobalvar` `/addglobalvar` | interpreter, **ephemeral** `ctx.globals` | not persisted (§3.4) |
...
4. **Chat vars persist; global vars are ephemeral.** `StCtx.setVar(key, v, 'local')` →
   `host.applyVariableOps([{ op: 'set', path: '/' + key.split('.').join('/'), value: v }])` (JSON-pointer; the
   interpreter already mutates `ctx.vars` in place first for within-run reads). `ctx.vars` is a **clone** of
   the cached `stat_data` so an in-run mutation can't corrupt the thRuntime cache — the real refresh flows
   back through `applyVariableOps` → `onVarsChanged`. `global` scope reads/writes only the per-run `ctx.globals`
   object (no `Host` global-var store exists; wiring the existing `pluginVars` global scope through the `Host`
   + WCV IPC is a future task). Defaulted to ephemeral to keep this domain IPC-free.
...
**No additions.** Every backing already exists (`statData` sync; `applyVariableOps`/`generate`/`generateRaw`
async; `setInput`). **One removal:** delete `triggerSlash(cmd): Promise<string>` from
[types.ts](../../../src/shared/thRuntime/types.ts) and the two adapter stubs.
...
**None.** That's the headline: because the interpreter runs in the shared runtime over already-parity `Host`
methods, neither `cardBridge/host.ts` nor `wcvHost.ts`/`wcvIpc.ts` gains code — they only **lose** the dead
`triggerSlash` stub. No `wcv-host-*` channel is added.
...
- `src/shared/thRuntime/types.ts` — remove `triggerSlash` from `Host`.
- `src/shared/thRuntime/index.ts` — `runTriggerSlash` helper + wire `triggerSlash`; drop the `host.triggerSlash`
  delegation.
- `src/renderer/src/cardBridge/host.ts`, `src/preload/wcvHost.ts` — remove the `triggerSlash` stub.
- `test/thRuntime.test.ts` — mock-Host: drop `triggerSlash`; add `triggerSlash` behavior tests.
...
1. **Global-var persistence** (§3.4) — defaulted to **ephemeral per run**. Flag if a card needs
   `/setglobalvar` to persist; that's a follow-up wiring the existing main-side `pluginVars` global scope
   through the `Host` + a WCV IPC.
...
4. **Remove `host.triggerSlash`** (§3.6) vs keep as an escape hatch — defaulted to **remove** (dead). Keep
   only if a future "delegate to the renderer registry / card-registered commands" path is wanted.
...
- Parity by construction: **no** new IPC / adapter code; `host.triggerSlash` removed from the seam + both
  adapters.
- New thRuntime tests pass; `npm test` + `typecheck` + `build` green; no new lint.
- Global-var persistence, loops/long-tail commands, card-registered-command dispatch, and `/send`-as-insert
  remain out of scope (noted).
